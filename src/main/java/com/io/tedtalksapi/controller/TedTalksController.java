...
import com.io.tedtalksapi.command.*;
...
public class TedTalksController {
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getTopInfluentialSpeakers(
        ...
        List<java.util.Map<String, Object>> result = new GetTopInfluentialSpeakersCommand(
    ...
    public ResponseEntity<ApiResponse<List<TedTalksRecordT>>> search(@RequestParam("value") String value) {
        ...
        List<TedTalksRecordT> results = new SearchTedTalksCommand(repository, value).execute();
        if (results.isEmpty()) {
    ...
    public ResponseEntity<ApiResponse<TedTalksRecordT>> saveTedTalkRecord(@Valid @RequestBody TedTalk body) {
        ...
        TedTalksRecordT created = new CreateTedTalkCommand(repository, mapper, body, validator).execute();
    ...
    public ResponseEntity<ApiResponse<TedTalksRecordT>> updateTedTalk(@PathVariable UUID id,
        ...
        TedTalksRecordT updated = new UpdateTedTalkCommand(repository, id, body).execute();
    ...
    public ResponseEntity<ApiResponse<Void>> deleteTedTalk(@PathVariable UUID id) {
        ...
        new DeleteTedTalkCommand(repository, id).execute();
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getMostInfluentialPerYear() {
        log.info("Request to get most influential TedTalk per year");
        List<Map<String, Object>> results = new GetMostInfluentialTedTalkPerYearCommand(
                repository).execute();
        return ResponseEntity.ok(ApiResponse.success(results));
    }
